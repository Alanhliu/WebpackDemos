...
const extractPageStyle = new ExtractTextPlugin({
    //这里的name是entry对象的key
    //所以分离出来的css文件是css目录下index.min.css,second.min.css
    filename: (getPath) => {
        return getPath('css/[name].page.min.css');
    },
    allChunks: true
});
const extractCommonStyle = new ExtractTextPlugin({
    filename: 'css/background.common.min.css',
    allChunks: true
});
...
module.exports = {
	...
	module: {
		...
		rules:[
			...
			{
				test: /(\.page.css|\.page.less)$/,
				use: extractPageStyle.extract({
					fallback: "style-loader",
					use: ["css-loader","less-loader"]
				})
			},
            {
                test: /(\.common.css|\.common.less)$/,
